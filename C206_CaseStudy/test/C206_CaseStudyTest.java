...
public class C206_CaseStudyTest extends C206CaseStudy {
	...
	public void setUp() throws Exception {
		...
		U3 = new User("Harry", "Designer", "Harry@gmail.com", "HarryDesign789", true);
...
//=======================Add Quotation Items ===========================================================
	...
	public void testRetrieveAllRequestedQuotation() {
		...
		String outputHDB = String.format("%-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s\n", "HDB",
				"100.25", "James", "91234567", "James@gmail.com", "1000.1", "2016-11-20", "2", "0", "", "false");
		...
		String outputPrivate = String.format("%-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s\n",
				"Private", "200.25", "Jacob", "98765432", "Jacob@hotmail.com", "200.2", "2016-02-13", "1", "1", "",
				"true");
...
//    } // Done by Jasrene
	...
	public void testRetrieveAllPackage() {
		...
		// Given an empty list, after adding 2 packages, test if the size of the list is
		// 2
		C206CaseStudy.addPackage(PackageList, P1);
		C206CaseStudy.addPackage(PackageList, P2);
		assertEquals("Test if that Package arraylist size is 2?", 2, PackageList.size());

		// test if the expected output string same as the list of packages retrieved
		// from the CaseStudy
		allPackage = C206CaseStudy.retrieveAllPackage(PackageList);

		testOutput = String.format("%-10s %-30s %-10s %-10s %-20.2f\n", "P101", "Shiny Kitchen", "2015-10-03",
				"2015-10-07", 400.00);
		testOutput += String.format("%-10s %-30s %-10s %-10s %-20.2f\n", "P102", "Small BedRoom", "2012-01-12",
				"2012-01-18", 500.00);
	...
	public void testDeleteUser() {
		assertNotNull("Test if there is User arraylist to add to", UserList);
	...
	public void testRetrieveAllUser() {
		assertNotNull("Test if there is a User arraylist to add to", UserList);

		C206CaseStudy.addUser(UserList, U1);
		C206CaseStudy.addUser(UserList, U2);
		C206CaseStudy.addUser(UserList, U3);
		assertEquals("Test if that the User arraylist size is 3?", 3, UserList.size());

		String allUser = C206CaseStudy.retrieveAllUser(UserList);
		String output = String.format("%-10s %-30s %-10s %-20s\n", "John", "Admin", "John@gmail.com", true);
		String output1 = String.format("%-10s %-30s %-10s %-20s\n", "Mary", "Customer", "Mary@gmail.com", true);
		String output2 = String.format("%-10s %-30s %-10s %-20s\n", "Harry", "Designer", "Harry@gmail.com", true);

		assertEquals("Check that ViewAllUserlist", allUser,output);
